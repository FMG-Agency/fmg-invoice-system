...
test("ships the locked Media Guide production workflow", async () => {
  ...
  assert.match(workOrder, /Add new bundle/);
  assert.match(workOrder, /Add bundles, then customize every detail/);
  assert.match(workOrder, /Bundle name/);
  assert.match(workOrder, /One per line/);
  assert.match(workOrder, /ORDER CREATED/);
  assert.match(workOrder, /Edit work order/);
  assert.match(workOrder, /Authorized manager correction/);
  assert.match(workOrder, /Final · editable/);
  assert.match(workOrder, /linked Draft invoice updates automatically/);
  ...
  assert.match(productionApi, /name: data\.bundles\[index\]\.name/);
  assert.match(productionApi, /inputs: data\.bundles\[index\]\.inputs/);
  assert.match(productionApi, /outputs: data\.bundles\[index\]\.outputs/);
  assert.match(productionApi, /action: z\.literal\("managerEdit"\)/);
  assert.match(productionApi, /Only an Operation Manager or administrator can edit work orders/);
  assert.match(productionApi, /without changing its approval stage/);
  assert.match(productionApi, /existing\.status === "Draft"/);
  assert.match(productionApi, /total = \? - COALESCE\(discount, 0\) \+ COALESCE\(tax, 0\)/);
...
test("ships the Production talent and crew directory with work-order pickers", async () => {
  ...
  assert.match(directory, /Model nationality/);
  assert.match(directory, /Hourly rate · EGP/);
  assert.match(directory, /Daily rate · EGP/);
  assert.match(directory, /member\.hourlyRate !== null/);
  assert.match(directoryStyles, /\.crewGrid/);
  assert.match(directoryStyles, /\.modelRates/);
  ...
  assert.match(workOrder, /modelRateSummary\(selectedMember\)/);
  assert.match(productionApi, /action: z\.literal\("saveCrew"\)/);
  assert.match(productionApi, /model_nationality AS modelNationality/);
  assert.match(productionApi, /hourly_rate AS hourlyRate/);
  assert.match(productionApi, /daily_rate AS dailyRate/);
...
test("upgrades existing production orders for Operations final approval", async () => {
  ...
  await db.executeMultiple(await readFile(new URL("drizzle/0019_odd_doctor_faustus.sql", root), "utf8"));
  ...
  const crewColumns = await db.execute("PRAGMA table_info(production_crew_members)");
  const crewColumnNames = crewColumns.rows.map((column) => column.name);
  assert.ok(crewColumnNames.includes("model_nationality"));
  assert.ok(crewColumnNames.includes("hourly_rate"));
  assert.ok(crewColumnNames.includes("daily_rate"));
  ...
  const seededModels = await db.execute("SELECT name, phone, notes, model_nationality AS modelNationality FROM production_crew_members WHERE category = 'model' ORDER BY id");
  assert.equal(seededModels.rows.length, 17);
  assert.ok(seededModels.rows.some((model) => model.name === "Neven Tarek" && model.notes === "Ahmed Attia" && model.modelNationality === "egyptian"));
  assert.ok(seededModels.rows.some((model) => model.name === "Anastasia" && model.notes === "Foreign model" && model.modelNationality === "foreign"));
  db.close();
});
