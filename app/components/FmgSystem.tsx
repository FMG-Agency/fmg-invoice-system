...
function DataPanel({ state, mutate, busy, showToast, editDocument, openWorkOrder }: { openWorkOrder: (id: number) => void; state: AppState; mutate: Mutation; busy: boolean; showToast: (message: string) => void; editDocument: (document: DocumentRecord) => void }) {
  ...
  const filtered = useMemo(() => state.documents.filter((document) => (tab === "all" || document.type === tab) && (category === "all" || String(document.categoryId) === category) && (status === "all" || document.status === status) && [document.generatedCode, document.clientName, document.companyName, document.categoryName, document.createdByName || "", document.productionWorkOrderId ? `WO-${String(document.productionWorkOrderId).padStart(4, "0")}` : ""].some((value) => value.toLowerCase().includes(query.toLowerCase()))).sort((a, b) => sort === "amount" ? b.total - a.total : sort === "client" ? (a.companyName || a.clientName).localeCompare(b.companyName || b.clientName) : new Date(b.date).getTime() - new Date(a.date).getTime()), [state.documents, tab, category, status, query, sort]);
