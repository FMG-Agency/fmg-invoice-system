  ...
  return <div className={styles.workspace}>
        ...
        return <article id={`task-${task.id}`} key={task.id} className={`${styles.taskCard} ${task.liveLateMinutes && task.status === "assigned" ? styles.overdueCard : ""}`}>
          ...
          {open && <div className={styles.taskDetails}>
            <div className={styles.detailGrid}>
              ...
              {(task.gridCells.length > 0 || task.gridNotes) && <section><span><FileText size={14} /> INSTAGRAM GRID</span><div className={styles.instagramGrid}>{task.gridCells.map((cell, index) => <div key={index} className={styles.gridCell} data-kind={cell}><small>#{index + 1}</small><strong>{cell === "design" ? "▧" : cell === "carousel" ? "▣" : "▶"}</strong><span>{cell}</span>{task.gridPostNotes?.[index] && <p className={styles.postNote}><b>Note</b>{task.gridPostNotes[index]}</p>}</div>)}</div>{task.gridNotes && <p>{task.gridNotes}</p>}</section>}
    ...
    {createOpen && <TaskModal eyebrow="ASSIGN NEW WORK" title="Create a detailed task" description={state.role === "account_manager" ? "Account Managers can assign to the team, excluding administrators and Operation Managers." : "Choose any internal user, define the work, and set an exact start and deadline."} onClose={() => !saving && setCreateOpen(false)}>
      <form className={styles.taskForm} onSubmit={(event) => void createTask(event)}>
        ...
        <section className={styles.gridEditor}><label className={styles.gridToggle}><input type="checkbox" checked={draft.gridCells.length>0} onChange={event=>setDraft(current=>({...current,gridCells:event.target.checked?Array(9).fill("design"):[],gridPostNotes:event.target.checked?Array(9).fill(""):[]}))}/> Add Instagram grid <small>Optional</small></label>{draft.gridCells.length>0 && <><p>Choose a content type for each post. Read left to right, row by row.</p><div className={styles.instagramGrid}>{draft.gridCells.map((cell,index)=><div key={index} className={styles.gridCell} data-kind={cell}><small>Post {index+1}</small><strong aria-hidden="true">{cell === "design" ? "▧" : cell === "carousel" ? "▣" : "▶"}</strong><select aria-label={`Post ${index+1} content type`} value={cell} onChange={event=>setDraft(current=>({...current,gridCells:current.gridCells.map((value,i)=>i===index?event.target.value as TaskGridCell:value)}))}><option value="design">Design</option><option value="carousel">Carousel</option><option value="video">Video</option></select><label className={styles.postNoteInput}><span>Note <small>Optional</small></span><textarea aria-label={`Post ${index+1} note`} rows={3} maxLength={2000} value={draft.gridPostNotes[index] || ""} onChange={event=>setDraft(current=>({...current,gridPostNotes:current.gridCells.map((_,i)=>i===index?event.target.value:current.gridPostNotes[i] || "")}))} placeholder="Write a note for this post…" /></label></div>)}</div><div className={styles.gridButtons}><button type="button" disabled={draft.gridCells.length>=36} onClick={()=>setDraft(current=>({...current,gridCells:[...current.gridCells,"design","design","design"],gridPostNotes:[...current.gridPostNotes,"","",""]}))}>Add row</button><button type="button" disabled={draft.gridCells.length<=3} onClick={()=>setDraft(current=>({...current,gridCells:current.gridCells.slice(0,-3),gridPostNotes:current.gridPostNotes.slice(0,-3)}))}>Remove last row</button></div></>}</section>
