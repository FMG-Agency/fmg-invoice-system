...
test("ships the complete product, protected access, HR payroll, and Vercel storage adapters", async () => {
  const [component, hrComponent, api, hrApi, hrImportApi, pdfApi, authApi, authServer, database, vercel, migration, authMigration, hrMigration] = await Promise.all([
  ...
  for (const expected of ["Import biometric Excel", "Payroll", "Policy settings", "Export payroll CSV", "Monthly commission"]) {
