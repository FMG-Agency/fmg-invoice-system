...
function DocumentDateInput({ value, onChange, label, required = false, min, max }: { value: string; onChange: (value: string) => void; label: string; required?: boolean; min?: string; max?: string }) {
  const [displayValue, setDisplayValue] = useState(() => formatDocumentDate(value));

  useEffect(() => {
    setDisplayValue(formatDocumentDate(value));
  }, [value]);
  ...
  return <div className="document-date-input">
    <input
      ...
      onBlur={() => setDisplayValue(formatDocumentDate(value))}
      aria-label={label}
    />
...
function DocumentPreview({ draft, client, category, code }: { draft: DocumentDraft; client?: Client; category?: Category; code: string }) {
  ...
  const catalogTable = (items: LineItem[], nameLabel: string) => <table className={cx("paper-package-table", draft.type === "invoice" && "invoice")}><thead><tr>{draft.type === "invoice" && <th>DATE</th>}<th>{nameLabel}</th><th>INPUTS</th><th>OUTPUTS</th><th>PRICE · {draft.currency}</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}>{draft.type === "invoice" && <td>{formatDocumentDate(item.date || draft.date) || "—"}</td>}<td><strong>{item.description || "—"}</strong>{item.kind === "addon" && item.appliesTo && <small>For {item.appliesTo}</small>}</td><td>{item.inputs.length ? item.inputs.join(" • ") : "—"}</td><td>{item.outputs.length ? item.outputs.join(" • ") : "—"}{item.kind === "addon" && item.bundleTotal !== null && <small>Bundle total: {item.bundleTotal.toLocaleString()} EGP</small>}</td><td>{(item.qty * item.unitPrice).toLocaleString()}</td></tr>)}</tbody></table>;
