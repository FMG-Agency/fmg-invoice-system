...
type AttendanceDraft = Pick<AttendanceRecord, "firstIn" | "lastOut" | "status" | "lateExcused" | "overtimeApproved" | "notes">;
...
export function AttendancePanel({ state, mutate, busy, onMonthChange, onStateChange, showToast }: { state: HrState; mutate: HrMutation; busy: boolean; onMonthChange: (month: string) => Promise<void>; onStateChange: (state: HrState) => void; showToast: (message: string) => void }) {
  ...
  function openAttendance(record: AttendanceRecord) {
    ...
    setAttendanceDraft({ firstIn: record.firstIn, lastOut: record.lastOut, status: record.status, lateExcused: record.lateExcused, overtimeApproved: record.overtimeApproved, notes: record.notes });
  }
  ...
  function exportAttendance() {
    downloadCsv(`FMG-attendance-${state.month}.csv`, [["Date", "Employee", "Biometric ID", "First in", "Last out", "All punches", "Status", "Late minutes", "Overtime minutes", "Late deduction", "Overtime pay", "Friday pay", "Notes"], ...filteredAttendance.map((record) => [record.workDate, record.employeeName, record.biometricCode, record.firstIn, record.lastOut, record.punches.join(" | "), readableStatus(record.status), record.lateMinutes, record.overtimeMinutes, record.lateDeduction, record.overtimePay, record.fridayPay, record.notes])]);
  }

  function exportPayroll() {
    downloadCsv(`FMG-payroll-${state.month}.csv`, [["Employee", "Title", "Base salary", "Fixed commission", "Other additions", "Overtime", "Friday work", "Late/absence deduction", "Fixed deduction", "Other deductions", "Net salary", "Late minutes", "Overtime minutes"], ...state.payroll.map((record) => [record.employeeName, record.title, record.baseSalary, record.monthlyCommission, record.manualAdditions, record.overtimePay, record.fridayPay, record.lateDeduction, record.monthlyDeduction, record.manualDeductions, record.netSalary, record.lateMinutes, record.overtimeMinutes])]);
  }
  ...
  return <>
    <section className="hr-commandbar panel">
      ...
      <div className="hr-command-actions"><button className="secondary-button" onClick={openPolicy}><Settings2 size={16} /> Policy settings</button><input ref={fileInput} className="sr-only" type="file" accept=".xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) void importWorkbook(file); }} /><button className="primary-button" onClick={() => fileInput.current?.click()} disabled={uploading}>{uploading ? <Clock3 size={16} /> : <FileSpreadsheet size={16} />}{uploading ? "Importing…" : "Import biometric Excel"}</button></div>
    </section>

    <section className="policy-strip"><CheckCircle2 size={18} /><div><strong>Office Policy is active</strong><span>Free arrival through {state.policy.freeArrivalUntil} · overtime after {state.policy.overtimeStartsAt} only when arrival is by {state.policy.overtimeArrivalCutoff} · Friday ×{state.policy.fridayMultiplier}</span></div><small>Salary day = salary ÷ {state.policy.salaryDivisor}</small></section>
    ...
    <section className="panel data-panel">
      ...
      {tab === "attendance" && <><div className="filters-row"><select value={employeeFilter} onChange={(event) => setEmployeeFilter(event.target.value)}><option value="all">All employees</option>{state.employees.map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="all">All statuses</option>{(["present", "incomplete", "absent", "friday", "vacation", "sick_leave", "urgent_leave", "normal_leave", "assignment"] as AttendanceStatus[]).map((status) => <option key={status} value={status}>{readableStatus(status)}</option>)}</select><div className="toolbar-spacer" /><button className="secondary-button" onClick={exportAttendance} disabled={!filteredAttendance.length}><Download size={15} /> Export CSV</button></div>{filteredAttendance.length ? <div className="table-scroll"><table className="data-table attendance-table"><thead><tr><th>Date</th><th>Employee</th><th>First in</th><th>Last out</th><th>Status</th><th>Late</th><th>Overtime</th><th>Payroll impact</th><th>Review</th></tr></thead><tbody>{filteredAttendance.map((record) => <tr key={record.id}><td><strong className="table-main">{record.workDate}</strong><small className="table-sub">{new Date(`${record.workDate}T12:00:00`).toLocaleDateString("en", { weekday: "short" })}</small></td><td><strong className="table-main">{record.employeeName}</strong><small className="table-sub">ID {record.biometricCode || "—"}</small></td><td>{record.firstIn || "—"}</td><td>{record.lastOut || "—"}</td><td><span className={cx("attendance-status", `attendance-${record.status}`)}>{readableStatus(record.status)}</span></td><td><strong className={record.lateMinutes ? "negative-value" : ""}>{minutesLabel(record.lateMinutes)}</strong>{record.lateExcused && <small className="table-sub">Excused</small>}</td><td><strong className={record.overtimeMinutes ? "positive-value" : ""}>{minutesLabel(record.overtimeMinutes)}</strong>{!record.overtimeApproved && <small className="table-sub">Not approved</small>}</td><td><span className="impact-add">+{hrMoney(record.overtimePay + record.fridayPay, state.policy.currency)}</span><span className="impact-minus">−{hrMoney(record.lateDeduction, state.policy.currency)}</span></td><td><button className="table-review" onClick={() => openAttendance(record)}><Pencil size={15} /> Review</button></td></tr>)}</tbody></table></div> : <div className="empty-panel"><div className="empty-icon"><FileSpreadsheet size={24} /></div><h3>No attendance records for this view</h3><p>Import the monthly biometric Excel file, or change the employee/status filters.</p><button className="small-primary" onClick={() => fileInput.current?.click()}><Plus size={15} /> Import Excel</button></div>}</>}

      {tab === "payroll" && <><div className="filters-row"><span className="record-count">Calculated from salary, policy, attendance, and monthly adjustments</span><div className="toolbar-spacer" /><button className="secondary-button" onClick={exportPayroll} disabled={!state.payroll.length}><Download size={15} /> Export payroll CSV</button></div>{state.payroll.length ? <div className="table-scroll"><table className="data-table payroll-table"><thead><tr><th>Employee</th><th>Base</th><th>Commission & additions</th><th>Overtime & Friday</th><th>Attendance deduction</th><th>Other deductions</th><th>Net salary</th><th>Details</th></tr></thead><tbody>{state.payroll.map((record) => <tr key={record.employeeId}><td><strong className="table-main">{record.employeeName}</strong><small className="table-sub">{record.title || "Title not set"}</small></td><td>{hrMoney(record.baseSalary, state.policy.currency)}</td><td className="positive-value">+{hrMoney(record.monthlyCommission + record.manualAdditions, state.policy.currency)}</td><td className="positive-value">+{hrMoney(record.overtimePay + record.fridayPay, state.policy.currency)}</td><td className="negative-value">−{hrMoney(record.lateDeduction, state.policy.currency)}</td><td className="negative-value">−{hrMoney(record.monthlyDeduction + record.manualDeductions, state.policy.currency)}</td><td><strong className="net-salary">{hrMoney(record.netSalary, state.policy.currency)}</strong></td><td><button className="table-review" onClick={() => openAdjustment(record.employeeId)}><Plus size={15} /> Adjustment</button><small className="table-sub">{minutesLabel(record.lateMinutes)} late · {minutesLabel(record.overtimeMinutes)} OT</small></td></tr>)}</tbody></table></div> : <div className="empty-panel"><div className="empty-icon"><BadgeDollarSign size={24} /></div><h3>Add employee salaries first</h3><p>Payroll summaries appear automatically for active employees.</p></div>}</>}
    ...
    {editingAttendance && attendanceDraft && <HrModal title={`Review ${editingAttendance.employeeName} · ${editingAttendance.workDate}`} description={`Raw punches: ${editingAttendance.punches.join(" · ") || "No punches"}`} onClose={() => setEditingAttendance(null)}>
      <form className="modal-form" onSubmit={saveAttendance}><div className="form-grid">
        ...
        <label className="field field-wide"><span>Attendance status</span><select value={attendanceDraft.status} onChange={(event) => setAttendanceDraft({ ...attendanceDraft, status: event.target.value as AttendanceStatus })}>{(["present", "incomplete", "absent", "friday", "vacation", "sick_leave", "urgent_leave", "normal_leave", "assignment"] as AttendanceStatus[]).map((status) => <option key={status} value={status}>{readableStatus(status)}</option>)}</select></label>
        <label className="check-field"><input type="checkbox" checked={attendanceDraft.lateExcused} onChange={(event) => setAttendanceDraft({ ...attendanceDraft, lateExcused: event.target.checked })} /><span>Manager approved / excuse lateness</span></label><label className="check-field"><input type="checkbox" checked={attendanceDraft.overtimeApproved} onChange={(event) => setAttendanceDraft({ ...attendanceDraft, overtimeApproved: event.target.checked })} /><span>Include eligible overtime in payroll</span></label>
        <label className="field field-wide"><span>Manager note</span><textarea value={attendanceDraft.notes} onChange={(event) => setAttendanceDraft({ ...attendanceDraft, notes: event.target.value })} /></label>
    ...
    {policyOpen && <HrModal title="Attendance & payroll policy" description="Prefilled from Office Policy.docx. Salary divisor and absence deductions are payroll settings because the document does not define them." onClose={() => setPolicyOpen(false)} wide>
      <form className="modal-form" onSubmit={savePolicy}><div className="form-grid hr-form-grid">
        ...
        <label className="field"><span>Workday minutes</span><input type="number" min="1" value={policyDraft.workdayMinutes} onChange={(event) => setPolicyDraft({ ...policyDraft, workdayMinutes: Number(event.target.value) })} /></label><label className="field"><span>Free arrival through</span><input type="time" value={policyDraft.freeArrivalUntil} onChange={(event) => setPolicyDraft({ ...policyDraft, freeArrivalUntil: event.target.value })} /></label>
        <label className="field"><span>Minute ×4 band ends</span><input type="time" value={policyDraft.minorLateUntil} onChange={(event) => setPolicyDraft({ ...policyDraft, minorLateUntil: event.target.value })} /></label><label className="field"><span>Quarter-day band ends</span><input type="time" value={policyDraft.quarterDayUntil} onChange={(event) => setPolicyDraft({ ...policyDraft, quarterDayUntil: event.target.value })} /></label>
        <label className="field"><span>Overtime starts</span><input type="time" value={policyDraft.overtimeStartsAt} onChange={(event) => setPolicyDraft({ ...policyDraft, overtimeStartsAt: event.target.value })} /></label><label className="field"><span>OT arrival cutoff</span><input type="time" value={policyDraft.overtimeArrivalCutoff} onChange={(event) => setPolicyDraft({ ...policyDraft, overtimeArrivalCutoff: event.target.value })} /></label>
        <label className="field"><span>Late minute multiplier</span><input type="number" min="0.01" step="0.01" value={policyDraft.minutePenaltyMultiplier} onChange={(event) => setPolicyDraft({ ...policyDraft, minutePenaltyMultiplier: Number(event.target.value) })} /></label><label className="field"><span>Overtime multiplier</span><input type="number" min="0.01" step="0.01" value={policyDraft.overtimeMultiplier} onChange={(event) => setPolicyDraft({ ...policyDraft, overtimeMultiplier: Number(event.target.value) })} /></label>
        <label className="field"><span>Friday multiplier</span><input type="number" min="0.01" step="0.01" value={policyDraft.fridayMultiplier} onChange={(event) => setPolicyDraft({ ...policyDraft, fridayMultiplier: Number(event.target.value) })} /></label><label className="field"><span>Absence day multiplier</span><input type="number" min="0" step="0.01" value={policyDraft.absenceDayMultiplier} onChange={(event) => setPolicyDraft({ ...policyDraft, absenceDayMultiplier: Number(event.target.value) })} /></label>
        <label className="check-field field-wide"><input type="checkbox" checked={policyDraft.absenceDeductionEnabled} onChange={(event) => setPolicyDraft({ ...policyDraft, absenceDeductionEnabled: event.target.checked })} /><span>Automatically deduct unapproved absence days (off by default because Office Policy does not state the payroll amount)</span></label>
      </div><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setPolicyOpen(false)}>Cancel</button><button className="primary-button" disabled={busy}>Save policy</button></div></form>
    </HrModal>}
    ...
    {adjustmentOpen && <HrModal title={`Add payroll adjustment · ${state.month}`} onClose={() => setAdjustmentOpen(false)}>
      <form className="modal-form" onSubmit={saveAdjustment}><div className="form-grid">
        ...
        <label className="field"><span>Label</span><input required value={adjustmentDraft.label} onChange={(event) => setAdjustmentDraft({ ...adjustmentDraft, label: event.target.value })} placeholder="e.g. Project commission" /></label><label className="field"><span>Amount ({state.policy.currency})</span><input required type="number" min="0.01" step="0.01" value={adjustmentDraft.amount} onChange={(event) => setAdjustmentDraft({ ...adjustmentDraft, amount: Number(event.target.value) })} /></label>
