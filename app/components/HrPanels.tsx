...
type OvertimeAllowanceDraft = { employeeId: number; workDate: string; kind: "normal" | "early"; note: string };
...
export function EmployeesPanel({ state, mutate, busy, showToast }: { state: HrState; mutate: HrMutation; busy: boolean; showToast: (message: string) => void }) {
  ...
  const [allowanceDraft, setAllowanceDraft] = useState<OvertimeAllowanceDraft>({ employeeId: 0, workDate: `${state.month}-01`, kind: "normal", note: "" });
  ...
  const overtimeAllowances = state.attendance.filter((record) => record.overtimeApproved || record.earlyOvertimeApproved).sort((left, right) => right.workDate.localeCompare(left.workDate));
  ...
  async function allowOvertimeDay(event: React.FormEvent) {
    event.preventDefault();
    try {
      await mutate({ action: "allowOvertimeDay", month: allowanceDraft.workDate.slice(0, 7), data: allowanceDraft });
      setAllowanceDraft((current) => ({ ...current, note: "" }));
      showToast(allowanceDraft.kind === "normal" ? "Normal overtime day allowed." : "Early overtime day allowed.");
    } catch (error) {
      showToast(error instanceof Error ? error.message : "Could not allow overtime for this day");
    }
  }
  ...
  return <>
    ...
    <section className="panel data-panel overtime-allowance-panel">
      <div className="overtime-allowance-heading"><div><span className="eyebrow">OVERTIME ALLOWANCE</span><h2>Allow overtime for a specific employee and day</h2><p>Only an allowed day and overtime type is included in payroll and the exported Excel. Normal overtime uses ×{state.policy.overtimeMultiplier}; early overtime uses ×{state.policy.earlyOvertimeMultiplier}.</p></div><CalendarClock size={25} /></div>
      <form className="overtime-allowance-form" onSubmit={allowOvertimeDay}>
        <label className="field"><span>Employee</span><select required value={allowanceDraft.employeeId || ""} onChange={(event) => setAllowanceDraft({ ...allowanceDraft, employeeId: Number(event.target.value) })}><option value="">Choose employee</option>{state.employees.filter((employee) => employee.active).map((employee) => <option key={employee.id} value={employee.id}>{employee.name}</option>)}</select></label>
        <label className="field"><span>Allowed day</span><input required type="date" value={allowanceDraft.workDate} onChange={(event) => setAllowanceDraft({ ...allowanceDraft, workDate: event.target.value })} /></label>
        <label className="field"><span>Overtime type</span><select value={allowanceDraft.kind} onChange={(event) => setAllowanceDraft({ ...allowanceDraft, kind: event.target.value as OvertimeAllowanceDraft["kind"] })}><option value="normal">Normal overtime · after {state.policy.overtimeStartsAt}</option><option value="early">Early overtime · before {state.policy.workdayStartsAt}</option></select></label>
        <label className="field overtime-note-field"><span>Reason / task</span><input required minLength={3} value={allowanceDraft.note} onChange={(event) => setAllowanceDraft({ ...allowanceDraft, note: event.target.value })} placeholder="Write the approved reason or assigned task" /></label>
        <button className="small-primary overtime-allow-button" disabled={busy}><CheckCircle2 size={16} /> Allow this day</button>
      </form>
      <div className="overtime-allowance-help"><CircleAlert size={16} /><span>Normal and early overtime are approved separately. To allow both on the same day, save one allowance for each type. Attendance punches remain the source of the actual minutes.</span></div>
      {overtimeAllowances.length > 0 && <div className="table-scroll"><table className="data-table overtime-allowance-table"><thead><tr><th>Employee</th><th>Day</th><th>Allowed overtime</th><th>Reason / task</th></tr></thead><tbody>{overtimeAllowances.map((record) => <tr key={record.id}><td><strong className="table-main">{record.employeeName}</strong><small className="table-sub">{record.biometricCode || "No biometric ID"}</small></td><td><strong className="table-main">{record.workDate}</strong></td><td><div className="approval-badges">{record.overtimeApproved && <span className="hr-status active">Normal ×{state.policy.overtimeMultiplier}</span>}{record.earlyOvertimeApproved && <span className="hr-status active">Early ×{state.policy.earlyOvertimeMultiplier}</span>}</div></td><td><span className="approval-note">{record.notes || "No reason recorded"}</span></td></tr>)}</tbody></table></div>}
    </section>
...
export function AttendancePanel({ state, mutate, busy, onMonthChange, onStateChange, showToast }: { state: HrState; mutate: HrMutation; busy: boolean; onMonthChange: (month: string) => Promise<void>; onStateChange: (state: HrState) => void; showToast: (message: string) => void }) {
  ...
  return <>
    ...
    <section className="policy-strip"><CheckCircle2 size={18} /><div><strong>FMG Office Policy v{state.policy.policyVersion} is active</strong><span>Only allowed days count · normal overtime ×{state.policy.overtimeMultiplier} after {state.policy.overtimeStartsAt} · early overtime ×{state.policy.earlyOvertimeMultiplier} before {state.policy.workdayStartsAt} · Friday ×{state.policy.fridayMultiplier}</span></div><small>Written reason / task required</small></section>
    ...
    <section className="panel data-panel">
      ...
      {tab === "attendance" && <>
        ...
        {filteredAttendance.length ? <div className="table-scroll"><table className="data-table attendance-table"><thead><tr><th>Date</th><th>Employee</th><th>First in</th><th>Last out</th><th>Status</th><th>Late / early</th><th>Overtime</th><th>Payroll impact</th><th>Review</th></tr></thead><tbody>{filteredAttendance.map((record) => <tr key={record.id}>
          ...
          <td><strong className={record.overtimeMinutes ? "positive-value" : ""}>{minutesLabel(record.overtimeMinutes)} total OT</strong>{record.normalOvertimeMinutes > 0 && <small className="table-sub">{minutesLabel(record.normalOvertimeMinutes)} normal → {record.normalMissionMinutes} mission min</small>}{record.earlyOvertimeMinutes > 0 && <small className="table-sub">{minutesLabel(record.earlyOvertimeMinutes)} early → {record.earlyMissionMinutes} mission min</small>}{record.totalMissionMinutes > 0 && <small className="table-sub"><strong>{record.totalMissionMinutes} total mission min</strong></small>}{record.lastOut > state.policy.overtimeStartsAt && !record.overtimeApproved && <small className="table-sub">Normal OT excluded · day not allowed</small>}{record.firstIn && record.firstIn < state.policy.workdayStartsAt && !record.earlyOvertimeApproved && <small className="table-sub">Early OT excluded · day not allowed</small>}</td>
    ...
    {policyOpen && <HrModal title="FMG attendance & leave policy" description="Policy v2 · late arrival, approved overtime, Friday work, leave notice periods, rewards, and deductions." onClose={() => setPolicyOpen(false)} wide>
      ...
      </div><div className="policy-reference"><div className="policy-rule-grid"><article><strong>Late arrival</strong><ul><li>Arrival through {policyDraft.freeArrivalUntil} is on time.</li><li>Until {policyDraft.minorLateUntil}: every late minute ×{policyDraft.minutePenaltyMultiplier}.</li><li>Then through {policyDraft.quarterDayUntil}: quarter-day deduction.</li><li>After {policyDraft.quarterDayUntil}: half day plus every extra minute ×{policyDraft.minutePenaltyMultiplier}.</li></ul></article><article><strong>Overtime & Friday</strong><ul><li>Only employee/day/type allowances saved with a written reason are included.</li><li>Days 1–16: allowed overtime after {policyDraft.overtimeStartsAt} counts at ×{policyDraft.overtimeMultiplier} even when arrival is after {policyDraft.overtimeArrivalCutoff}.</li><li>From day 17: allowed normal overtime also requires arrival by {policyDraft.overtimeArrivalCutoff}.</li><li>Allowed early overtime before {policyDraft.workdayStartsAt} counts at ×{policyDraft.earlyOvertimeMultiplier}.</li><li>Friday work is paid as {policyDraft.fridayMultiplier} normal days.</li></ul></article><article><strong>Leaves</strong><ul><li>Friday and religious occasions are off.</li><li>Resort leave: up to {policyDraft.resortLeaveDays} days, with {policyDraft.resortNoticeDays} days’ notice.</li><li>Sick leave over {policyDraft.sickReportAfterDays} days requires a medical report.</li><li>Urgent leave by {policyDraft.urgentLeaveDeadline}, maximum {policyDraft.urgentLeaveYearLimit} per year; normal leave needs {policyDraft.normalLeaveNoticeDays} days’ notice.</li></ul></article><article><strong>Rewards & deductions</strong><ul><li>Rewards: perfect performance and extra tasks.</li><li>Audit responsibility: creator → account manager + creator → operation manager + account manager + creator.</li><li>Incomplete tasks, weak performance, and work absence without written approval can be deducted.</li><li>Apply each amount from Payroll Adjustments with a written note.</li></ul></article></div></div><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setPolicyOpen(false)}>Cancel</button><button className="primary-button" disabled={busy}>Save policy</button></div></form>
    </HrModal>}
