...
export function FmgSystem() {
  ...
  function openLinkedWorkOrder(id: number) {
    if (!canOpenView("work-order")) return showToast("This account does not have access to work orders.");
    chooseView("work-order");
    setLinkedWorkOrderId(id);
  }
  ...
  if (auth.clientId !== null) return <><ReadingSize key={auth.userId} userId={auth.userId} /><ClientPortalShell displayName={auth.displayName || auth.username} dark={dark} onToggleTheme={toggleTheme} onLogout={logout} showToast={showToast} />{toast && <div className="toast" role="status">{toast}</div>}</>;

  return (
    <div className={cx("app-shell", desktopMenuClosed && "desktop-menu-closed")} dir="ltr">
      <ReadingSize key={auth.userId} userId={auth.userId} />
      ...
      <main className="main-area" dir="ltr">
        ...
        <div className="page-wrap">
          ...
          {view === "work-order" && <WorkOrderPanel initialOrderId={linkedWorkOrderId} showToast={showToast} onWorkspaceChanged={() => loadWorkspace(auth)} onOpenDraftInvoice={(invoiceId) => {
...
function HomePage({ name, role, chooseView, canOpenView }: { name: string; role: string; chooseView: (view: View) => void; canOpenView: (view: View) => boolean }) {
  ...
  return <div className="home-page">
    <section className="home-hero">
      ...
      <div className="home-art" aria-hidden="true"><div className="home-orbit home-orbit-one" /><div className="home-orbit home-orbit-two" /><span className="home-art-label">SUPERHEROES WHO CREATE</span><div className="home-brand-logo"><Image src="/fmg-logo-light.png" alt="FMG Agency" width={1760} height={712} priority unoptimized /></div><div className="home-art-star"><Sparkles size={42} /></div><span className="home-art-bottom">CREATE. COLLABORATE. DELIVER.</span></div>
    </section>
...
function DocumentEditor({ type, companyKey, state, mutate, busy, editing, onDone, showToast, openWorkOrder }: { openWorkOrder: (id: number) => void; type: "invoice" | "quotation"; companyKey: CompanyKey; state: AppState; mutate: Mutation; busy: boolean; editing: DocumentRecord | null; onDone: () => void; showToast: (message: string) => void }) {
  ...
  return <div className={cx("editor-layout", !previewVisible && "preview-hidden")}>
    <section className="panel editor-panel">
      <div className="editor-preview-controls"><button type="button" className="secondary-button" onClick={() => setPreviewVisible((visible) => !visible)} aria-expanded={previewVisible} aria-controls="document-live-preview">{previewVisible ? <EyeOff size={16} /> : <Eye size={16} />}{previewVisible ? "Hide live preview" : "Show live preview"}</button></div>
      ...
      {editing && <div className="document-origin"><span>Created by <strong>{editing.createdByName || "Not recorded"}</strong>{editing.productionWorkOrderId ? " · From work-order approval" : ""}</span>{editing.productionWorkOrderId && <button className="secondary-button" onClick={() => openWorkOrder(editing.productionWorkOrderId!)}><ClipboardList size={16} /> WO-{String(editing.productionWorkOrderId).padStart(4, "0")}</button>}</div>}
    ...
    {previewVisible && <aside id="document-live-preview" className="preview-panel"><div className="preview-toolbar"><span><Eye size={16} /> Live preview</span><span>A4</span></div><DocumentPreview draft={draft} client={client} category={category} code={provisionalCode} /></aside>}
  </div>;
}
...
function DataPanel({ state, mutate, busy, showToast, editDocument, openWorkOrder }: { openWorkOrder: (id: number) => void; state: AppState; mutate: Mutation; busy: boolean; showToast: (message: string) => void; editDocument: (document: DocumentRecord) => void }) {
  ...
  const searched = useMemo(() => [...state.documents, ...(state.deletedDocuments || [])].filter((document) => [document.generatedCode, document.clientName, document.companyName, document.categoryName, document.createdByName || "", document.productionWorkOrderId ? `WO-${String(document.productionWorkOrderId).padStart(4, "0")}` : ""].some((value) => value.toLowerCase().includes(query.trim().toLowerCase()))), [state.documents, state.deletedDocuments, query]);
  const matchesTab = (document: DocumentRecord) => tab === "all" || document.type === tab;
  const matchesCategory = (document: DocumentRecord) => category === "all" || String(document.categoryId) === category;
  const matchesStatus = (document: DocumentRecord) => status === "all" || document.status === status;
  const categoryOptions = searched.filter((document) => matchesTab(document) && matchesStatus(document));
  const statusOptions = searched.filter((document) => matchesTab(document) && matchesCategory(document));
  const tabOptions = searched.filter((document) => matchesCategory(document) && matchesStatus(document));
  const filtered = searched.filter((document) => matchesTab(document) && matchesCategory(document) && matchesStatus(document)).sort((a, b) => sort === "amount" ? b.total - a.total : sort === "client" ? (a.companyName || a.clientName).localeCompare(b.companyName || b.clientName) : new Date(b.date).getTime() - new Date(a.date).getTime());
  ...
  async function remove(document: DocumentRecord) { if (!window.confirm(`Delete ${document.generatedCode}? Its serial will remain visible as a deleted record.`)) return; try { await mutate({ action: "deleteDocument", id: document.id }); showToast("Document deleted. Its serial has been preserved."); } catch (error) { showToast(error instanceof Error ? error.message : "Could not delete document"); } }
  return <><section className="panel data-panel"><div className="data-tabs"><button className={tab === "all" ? "active" : ""} onClick={() => setTab("all")}>All documents <span>{tabOptions.length}</span></button><button className={tab === "invoice" ? "active" : ""} onClick={() => setTab("invoice")}>Invoices <span>{tabOptions.filter((item) => item.type === "invoice").length}</span></button><button className={tab === "quotation" ? "active" : ""} onClick={() => setTab("quotation")}>Quotations <span>{tabOptions.filter((item) => item.type === "quotation").length}</span></button></div><div className="filters-row"><div className="filter-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search code, client or creator" /></div><select aria-label="Filter documents by category" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All categories - {categoryOptions.length}</option>{state.categories.map((record) => <option key={record.id} value={record.id}>{record.name} - {categoryOptions.filter((document) => document.categoryId === record.id).length}</option>)}</select><select aria-label="Filter documents by status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses - {statusOptions.length}</option>{["Draft", "Sent", "Approved", "Paid", "Rejected", "Deleted"].map((record) => <option key={record} value={record}>{record} - {statusOptions.filter((document) => document.status === record).length}</option>)}</select><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="newest">Newest first</option><option value="amount">Highest value</option><option value="client">Client A–Z</option></select></div><div className="archive-results-summary" role="status" aria-live="polite">Total: <strong>{filtered.length}</strong><span>{filtered.length === 1 ? "document" : "documents"} shown</span></div>{filtered.length ? <div className="table-scroll"><table className="data-table document-table"><thead><tr><th scope="col">#</th><th>Document</th><th>Created by</th><th>Client</th><th>Category</th><th>Date</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.map((document, index) => <tr key={document.id} className={document.deletedAt ? "deleted-document-row" : ""}><td className="document-row-number">{index + 1}</td><td><div className="document-code"><span className={document.type}>{document.type === "invoice" ? <ReceiptText size={17} /> : <FileText size={17} />}</span><span><strong>{document.generatedCode}</strong>{document.deletedAt && <small>Deleted · serial reserved</small>}<small>{companyNames[document.companyKey || "fmg"]} · {document.type}</small></span></div></td><td><strong className="table-main">{document.createdByName || "Not recorded"}</strong>{document.productionWorkOrderId && <small className="table-sub">From work-order approval</small>}</td><td><strong className="table-main">{document.companyName || document.clientName}</strong><small className="table-sub">{document.ownerName}</small></td><td><span className="category-pill">{document.categoryPrefix}</span> {document.categoryName}</td><td>{prettyDate(document.date)}</td><td><strong className="table-main">{money(document.total, document.currency)}</strong></td><td>{document.deletedAt ? <span>Deleted</span> : <select className="status-select" value={document.status} onChange={(event) => setDocumentStatus(document, event.target.value)} disabled={busy}>{["Draft", "Sent", "Approved", "Paid", "Rejected"].map((record) => <option key={record}>{record}</option>)}</select>}</td><td>{document.deletedAt ? <span>Deleted record</span> : <div className="document-actions">{document.productionWorkOrderId && <button className="linked-work-order" onClick={() => openWorkOrder(document.productionWorkOrderId!)} title="Open linked work order"><ClipboardList size={16} /> WO-{String(document.productionWorkOrderId).padStart(4, "0")}</button>}<button onClick={() => setPreviewDocument(document)} title="Preview"><Eye size={16} /></button><a href={`/api/pdf/${document.id}?download=1`} title="Download"><Download size={16} /></a><button onClick={() => editDocument(document)} title="Edit"><Pencil size={16} /></button><button className="danger" onClick={() => remove(document)} title="Delete"><Trash2 size={16} /></button></div>}</td></tr>)}</tbody></table></div> : <EmptyPanel icon={FolderKanban} title={state.documents.length ? "No documents match these filters" : "Your archive is ready"} body={state.documents.length ? "Clear a filter or try a different search." : "Every generated invoice and quotation will be permanently stored here."} />}</section>{previewDocument && <DocumentPdfPreview key={previewDocument.id} id={previewDocument.id} code={previewDocument.generatedCode} onClose={() => setPreviewDocument(null)} />}</>;
}
