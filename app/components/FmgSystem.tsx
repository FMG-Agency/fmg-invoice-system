...
function DataPanel({ state, mutate, busy, showToast, editDocument, openWorkOrder }: { openWorkOrder: (id: number) => void; state: AppState; mutate: Mutation; busy: boolean; showToast: (message: string) => void; editDocument: (document: DocumentRecord) => void }) {
  ...
  const searched = useMemo(() => state.documents.filter((document) => [document.generatedCode, document.clientName, document.companyName, document.categoryName, document.createdByName || "", document.productionWorkOrderId ? `WO-${String(document.productionWorkOrderId).padStart(4, "0")}` : ""].some((value) => value.toLowerCase().includes(query.trim().toLowerCase()))), [state.documents, query]);
  ...
  return <><section className="panel data-panel"><div className="data-tabs"><button className={tab === "all" ? "active" : ""} onClick={() => setTab("all")}>All documents <span>{tabOptions.length}</span></button><button className={tab === "invoice" ? "active" : ""} onClick={() => setTab("invoice")}>Invoices <span>{tabOptions.filter((item) => item.type === "invoice").length}</span></button><button className={tab === "quotation" ? "active" : ""} onClick={() => setTab("quotation")}>Quotations <span>{tabOptions.filter((item) => item.type === "quotation").length}</span></button></div><div className="filters-row"><div className="filter-search"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search code, client or creator" /></div><select aria-label="Filter documents by category" value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">All categories - {categoryOptions.length}</option>{state.categories.map((record) => <option key={record.id} value={record.id}>{record.name} - {categoryOptions.filter((document) => document.categoryId === record.id).length}</option>)}</select><select aria-label="Filter documents by status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses - {statusOptions.length}</option>{["Draft", "Sent", "Approved", "Paid", "Rejected"].map((record) => <option key={record} value={record}>{record} - {statusOptions.filter((document) => document.status === record).length}</option>)}</select><select value={sort} onChange={(event) => setSort(event.target.value)}><option value="newest">Newest first</option><option value="amount">Highest value</option><option value="client">Client A–Z</option></select></div><div className="archive-results-summary" role="status" aria-live="polite">Total: <strong>{filtered.length}</strong><span>{filtered.length === 1 ? "document" : "documents"} shown</span></div>{filtered.length ? <div className="table-scroll"><table className="data-table document-table"><thead><tr><th scope="col">#</th><th>Document</th><th>Created by</th><th>Client</th><th>Category</th><th>Date</th><th>Amount</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.map((document, index) => <tr key={document.id}><td className="document-row-number">{index + 1}</td><td><div className="document-code"><span className={document.type}>{document.type === "invoice" ? <ReceiptText size={17} /> : <FileText size={17} />}</span><span><strong>{document.generatedCode}</strong><small>{companyNames[document.companyKey || "fmg"]} · {document.type}</small></span></div></td><td><strong className="table-main">{document.createdByName || "Not recorded"}</strong>{document.productionWorkOrderId && <small className="table-sub">From work-order approval</small>}</td><td><strong className="table-main">{document.companyName || document.clientName}</strong><small className="table-sub">{document.ownerName}</small></td><td><span className="category-pill">{document.categoryPrefix}</span> {document.categoryName}</td><td>{prettyDate(document.date)}</td><td><strong className="table-main">{money(document.total, document.currency)}</strong></td><td><select className="status-select" value={document.status} onChange={(event) => setDocumentStatus(document, event.target.value)} disabled={busy}>{["Draft", "Sent", "Approved", "Paid", "Rejected"].map((record) => <option key={record}>{record}</option>)}</select></td><td><div className="document-actions">{document.productionWorkOrderId && <button className="linked-work-order" onClick={() => openWorkOrder(document.productionWorkOrderId!)} title="Open linked work order"><ClipboardList size={16} /> WO-{String(document.productionWorkOrderId).padStart(4, "0")}</button>}<button onClick={() => setPreviewDocument(document)} title="Preview"><Eye size={16} /></button><a href={`/api/pdf/${document.id}?download=1`} title="Download"><Download size={16} /></a><button onClick={() => editDocument(document)} title="Edit"><Pencil size={16} /></button><button className="danger" onClick={() => remove(document)} title="Delete"><Trash2 size={16} /></button></div></td></tr>)}</tbody></table></div> : <EmptyPanel icon={FolderKanban} title={state.documents.length ? "No documents match these filters" : "Your archive is ready"} body={state.documents.length ? "Clear a filter or try a different search." : "Every generated invoice and quotation will be permanently stored here."} />}</section>{previewDocument && <DocumentPdfPreview key={previewDocument.id} id={previewDocument.id} code={previewDocument.generatedCode} onClose={() => setPreviewDocument(null)} />}</>;
}
